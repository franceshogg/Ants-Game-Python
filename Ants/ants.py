...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self): #, lower_bound=0, upper_bound=float('inf')):
        ...
        """
        step_counter = 0 
        if len(self.place.bees) != 0 and self.place.is_hive == False and self.lower_bound == 0:
            return random_bee(self.place.bees)
        else:
            current = self.place
            while len(current.bees) == 0 or current.is_hive == True or step_counter < self.lower_bound:  
                current = current.entrance
                step_counter += 1
                if self.upper_bound < step_counter:
                        return None
                if current.entrance == None:
                    return None
                if current.is_hive == True:
                    return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        """Reduces health by AMOUNT, and removes the FireAnt from its place if it
        has no health remaining.
        """
        for b in self.place.bees[:]:
            b.reduce_health(amount)
        if self.health - amount <= 0:
            for c in self.place.bees[:]:
                c.reduce_health(self.damage)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate): 
        if len(self.place.bees) != 0 and self.turns_to_chew <= 0:
            ...
            Ant.reduce_health(chosen_bee, chosen_bee.health)
...
class QueenAnt(ScubaThrower):
    ...
    def reduce_health(self, amount):
        """Reduces health by AMOUNT, and if the QueenAnt has no health
        remaining, signals the end of the game.
        """
        super().reduce_health(amount)
...
class SlowThrower(ThrowerAnt):
    """ThrowerAnt that throws a 'Slower' on Bees."""

    name = 'Slow'
    food_cost = 6
    damage = 0
    implemented = True  # Set to True to use in GUI

    def throw_at(self, target):
        if target is not None:
            self.apply_slow(target, 3)  # Slow effect for 3 turns

    def apply_slow(self, bee, turns):
